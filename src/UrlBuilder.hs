...
generateUlrBuilderMethod :: UlrBuilderMethod -> MemberDeclaration
generateUlrBuilderMethod (UlrBuilderMethod methodName (UrlBuilder parts queryParts)) = 
    mkMethodMemberDeclaration [Public] (mkTypeNamed "UrlBuilder") methodName params body
    where
...
invokeUrlBuilderMethod (UlrBuilderMethod methodName urlBuilder) = mkInvocationSimpleName methodName $ urlBuilderArgs
    where 
...
extractNamesFromUrlBuilder (UrlBuilder parts queryParts) = catMaybes $ concat [map extractNameFromUrlPart parts, map extractNameFromUrlQueryPart queryParts]
